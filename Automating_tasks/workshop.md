...
We can start by condensing all the above commands together.
    ...
    sort -o accessioned.csv > sorted.csv
...
Let's redirect the output from `sort` into the `uniq` function, and the output from `uniq` into the `grep` function. Note that because we now want the sort function to direct to the standard output and not a file, we need to remove the `-o` flag.
